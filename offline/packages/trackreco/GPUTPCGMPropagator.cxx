...
int GPUTPCGMPropagator::RotateToAlpha(float newAlpha)
{
  ...
  if (fabs(mT->GetSinPhi()) >= mMaxSinPhi || fabs(px0) < (1 - mMaxSinPhi)) {
    return -1;
  }
  ...
  if (fabs(py1) > mMaxSinPhi * mT0.GetPt() || fabs(px1) < (1 - mMaxSinPhi)) {
    return -1;
  }
...
{
  if (fabs(posAlpha - mAlpha) > 1.e-4f) {
...
int GPUTPCGMPropagator::FollowLinearization(const GPUTPCGMPhysicalTrackModel& t0e, float Bz, float dLp, bool inFlyDirection)
{
  ...
  if (mT->NDF() >= 15 && fabs(newSinPhi) > GPUCA_MAX_SIN_PHI) {
    return -4;
  }
  ...
  float dLmask = 0.f;
  bool maskMS = (fabs(dL) < mMaterial.DLMax);
  if (maskMS) {
    dLmask = dL;
  }
...
float GPUTPCGMPropagator::PredictChi2(float posY, float posZ, float err2Y, float err2Z) const
{
  ...
  if (!mFitInProjections || mT->NDF() <= 0) {
    ...
    return fabs((w0 * z0 + w1 * z1) * z0) + fabs((w1 * z0 + w2 * z1) * z1);
  }
}
...
int GPUTPCGMPropagator::Update(float posY, float posZ, int iRow, const GPUParam& param, short clusterState, bool rejectChi2, bool refit)
{
  ...
  if (mT->NDF() == -5) { // first measurement: no need to filter, as the result is known in advance. just set it.
    ...
    float* mP = mT->Par();
    if (refit) {
      mC[14] = std::max(0.5f, (float)fabs(mP[4]));
      mC[5] = std::max(0.2f, (float)fabs(mP[2]) / 2);
      mC[9] = std::max(0.5f, (float)fabs(mP[3]) / 2);
    }
    mP[0] = posY;
    mP[1] = posZ;
    mC[0] = err2Y;
...
int GPUTPCGMPropagator::Update(float posY, float posZ, short clusterState, bool rejectChi2, float err2Y, float err2Z)
{
  ...
  if (mFitInProjections || mT->NDF() <= 0) {
    ...
    chiY = fabs((w0 * z0 + w1 * z1) * z0);
    chiZ = fabs((w1 * z0 + w2 * z1) * z1);
  }
