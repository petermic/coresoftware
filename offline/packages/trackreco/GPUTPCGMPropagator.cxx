...
void GPUTPCGMPropagator::GetBxByBz(float Alpha, float X, float Y, float Z, float B[3]) const
{
...
float GPUTPCGMPropagator::GetBz(float Alpha, float X, float Y, float Z) const
{
  if (mToyMCEvents) { // special treatment for toy monte carlo
    float B[3];
...
int GPUTPCGMPropagator::RotateToAlpha(float newAlpha)
{
  ...
  float cc = cos(newAlpha - mAlpha);
  float ss = sin(newAlpha - mAlpha);
  ...
  if (abs(mT->GetSinPhi()) >= mMaxSinPhi || abs(px0) < (1 - mMaxSinPhi)) {
    return -1;
  }
  ...
  if (abs(py1) > mMaxSinPhi * mT0.GetPt() || abs(px1) < (1 - mMaxSinPhi)) {
    return -1;
  }
  ...
  mT->SinPhi() = -sqrt(1.f - mT->SinPhi() * mT->SinPhi()) * ss + mT->SinPhi() * cc;
...
int GPUTPCGMPropagator::PropagateToXAlpha(float posX, float posAlpha, bool inFlyDirection)
{
  if (abs(posAlpha - mAlpha) > 1.e-4f) {
...
int GPUTPCGMPropagator::PropagateToXAlphaBz(float posX, float posAlpha, bool inFlyDirection)
{
  if (abs(posAlpha - mAlpha) > 1.e-4) {
    if (RotateToAlpha(posAlpha) != 0) {
      return -2;
    }
  }
...
int GPUTPCGMPropagator::FollowLinearization(const GPUTPCGMPhysicalTrackModel& t0e, float Bz, float dLp, bool inFlyDirection)
{
  ...
  if (mT->NDF() >= 15 && abs(newSinPhi) > GPUCA_MAX_SIN_PHI) {
    return -4;
  }
  ...
  float dLmask = 0.f;
  bool maskMS = (abs(dL) < mMaterial.DLMax);
  if (maskMS) {
    dLmask = dL;
  }
  float dLabs = abs(dLmask);
...
int GPUTPCGMPropagator::GetPropagatedYZ(float x, float& projY, float& projZ)
{
...
void GPUTPCGMPropagator::GetErr2(float& err2Y, float& err2Z, const GPUParam& param, float posZ, int iRow, short clusterState) const
{
  if (!mSeedingErrors) {
...
float GPUTPCGMPropagator::PredictChi2(float posY, float posZ, int iRow, const GPUParam& param, short clusterState) const
{
  float err2Y, err2Z;
...
float GPUTPCGMPropagator::PredictChi2(float posY, float posZ, float err2Y, float err2Z) const
{
  ...
  if (!mFitInProjections || mT->NDF() <= 0) {
    ...
    return abs((w0 * z0 + w1 * z1) * z0) + abs((w1 * z0 + w2 * z1) * z1);
  }
}

int GPUTPCGMPropagator::Update(float posY, float posZ, int iRow, const GPUParam& param, short clusterState, bool rejectChi2, bool refit)
{
  float err2Y, err2Z;
  GetErr2(err2Y, err2Z, param, posZ, iRow, clusterState);
  ...
  if (mT->NDF() == -5) { // first measurement: no need to filter, as the result is known in advance. just set it.
    ...
    float* mP = mT->Par();
    if (refit) {
      mC[14] = max(0.5f, abs(mP[4]));
      mC[5] = max(0.2f, abs(mP[2]) / 2);
      mC[9] = max(0.5f, abs(mP[3]) / 2);
    }
    mP[0] = posY;
    mP[1] = posZ;
    mC[0] = err2Y;
...
int GPUTPCGMPropagator::Update(float posY, float posZ, short clusterState, bool rejectChi2, float err2Y, float err2Z)
{
  float* mC = mT->Cov();
  float* mP = mT->Par();
  ...
  if (mFitInProjections || mT->NDF() <= 0) {
    ...
    chiY = abs((w0 * z0 + w1 * z1) * z0);
    chiZ = abs((w1 * z0 + w2 * z1) * z1);
  }
...
float GPUTPCGMPropagator::ApproximateBetheBloch(float beta2)
{
  //------------------------------------------------------------------
...
void GPUTPCGMPropagator::CalculateMaterialCorrection()
{
  //*!
...
void GPUTPCGMPropagator::Rotate180()
{
  mT0.X() = -mT0.X();
  mT0.Y() = -mT0.Y();
  mT0.Q() = -mT0.Q();
...
void GPUTPCGMPropagator::ChangeDirection()
{
  mT0.Py() = -mT0.Py();
  mT0.Pz() = -mT0.Pz();
  mT0.Q() = -mT0.Q();
...
void GPUTPCGMPropagator::Mirror(bool inFlyDirection)
{
...
o2::base::MatBudget GPUTPCGMPropagator::getMatBudget(float* p1, float* p2)
{
