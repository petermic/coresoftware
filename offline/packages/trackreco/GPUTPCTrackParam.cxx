...
float GPUTPCTrackParam::GetDist2(const GPUTPCTrackParam& t) const
{
...
float GPUTPCTrackParam::GetDistXZ2(const GPUTPCTrackParam& t) const
{
...
float GPUTPCTrackParam::GetS(float x, float y, float Bz) const
{
...
void GPUTPCTrackParam::GetDCAPoint(float x, float y, float z, float& xp, float& yp, float& zp, float Bz) const
{
...
bool GPUTPCTrackParam::TransportToX(float x, GPUTPCTrackLinearisation& t0, float Bz, float maxSinPhi, float* DL)
{
...
bool GPUTPCTrackParam::TransportToX(float x, float sinPhi0, float cosPhi0, float Bz, float maxSinPhi)
{
...
bool GPUTPCTrackParam::TransportToX(float x, float Bz, float maxSinPhi)
{
...
bool GPUTPCTrackParam::TransportToXWithMaterial(float x, GPUTPCTrackLinearisation& t0, GPUTPCTrackFitParam& par, float Bz, float maxSinPhi)
{
...
bool GPUTPCTrackParam::TransportToXWithMaterial(float x, GPUTPCTrackFitParam& par, float Bz, float maxSinPhi)
{
  //* Transport the track parameters to X=x  taking into account material budget
...
bool GPUTPCTrackParam::TransportToXWithMaterial(float x, float Bz, float maxSinPhi)
{
  //* Transport the track parameters to X=x taking into account material budget
...
//*
float GPUTPCTrackParam::BetheBlochGeant(float bg2, float kp0, float kp1, float kp2, float kp3, float kp4)
{
  //
  ...
  //*** Density effect
  float d2 = 0.f;
  const float x = 0.5f * log(bg2);
  const float lhwI = log(28.816f * 1e-9f * std::sqrt(rho * mZA) / mI);
  if (x > x1) {
    d2 = lhwI + x - 0.5f;
  } else if (x > x0) {
...
float GPUTPCTrackParam::BetheBlochSolid(float bg)
{
  //------------------------------------------------------------------
...
float GPUTPCTrackParam::BetheBlochGas(float bg)
{
  //------------------------------------------------------------------
  // This is an approximation of the Bethe-Bloch formula,
...
float GPUTPCTrackParam::ApproximateBetheBloch(float beta2)
{
  //------------------------------------------------------------------
...
void GPUTPCTrackParam::CalculateFitParameters(GPUTPCTrackFitParam& par, float mass)
{
  //*!
...
bool GPUTPCTrackParam::CorrectForMeanMaterial(float xOverX0, float xTimesRho, const GPUTPCTrackFitParam& par)
{
  //------------------------------------------------------------------
...
//* Rotation
//*
bool GPUTPCTrackParam::Rotate(float alpha, float maxSinPhi)
{
...
bool GPUTPCTrackParam::Rotate(float alpha, GPUTPCTrackLinearisation& t0, float maxSinPhi)
{
  //* Rotate the coordinate system in XY on the angle alpha
...
bool GPUTPCTrackParam::Filter(float y, float z, float err2Y, float err2Z, float maxSinPhi, bool paramOnly)
{
